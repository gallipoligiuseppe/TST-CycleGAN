...
sys.path.append("")
from helper import evaluate_sc
...
def main():
    ...
    parser.add_argument('-dataset_path', default='', type=str, help='the path of dataset')
    ...
    parser.add_argument('-embed_path', default=None, type=str, help='the path of embedding')
    ...
    with open('{}train.0'.format(opt.dataset_path),'r') as f:
        for line in f.readlines():
            if opt.lowercase:
                line = line.lower()
            train_src.append(tokenizer.encode(line.strip())[:opt.max_len])
    with open('{}train.1'.format(opt.dataset_path),'r') as f:
        for line in f.readlines():
            if opt.lowercase:
                line = line.lower()
            train_tgt.append(tokenizer.encode(line.strip())[:opt.max_len])
    with open('{}dev.0'.format(opt.dataset_path),'r') as f:
        for line in f.readlines():
            if opt.lowercase:
                line = line.lower()
            valid_src.append(tokenizer.encode(line.strip())[:opt.max_len])
    with open('{}dev.1'.format(opt.dataset_path),'r') as f:
        for line in f.readlines():
            if opt.lowercase:
                line = line.lower()
    ...
    if os.path.exists('./textcnn/checkpoints/embedding.pt'):
        embedding = torch.load('./textcnn/checkpoints/embedding.pt')
    else:
        embed_path = f'{opt.embed_path}glove.840B.300d.txt'
        embedding = load_embedding(tokenizer, 300, embed_path)
        torch.save(embedding, './textcnn/checkpoints/embedding.pt')
        ...
        for idx, batch in enumerate(train_loader):
            ...
            if optimizer.steps % opt.eval_step == 0:
                ...
                if avg_acc < valid_acc:
                    avg_acc = valid_acc
                    save_path = './textcnn/checkpoints/textcnn_{}.chkpt'.format(opt.dataset)
